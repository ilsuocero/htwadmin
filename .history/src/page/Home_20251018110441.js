...
function Home() {
  ...
  // Use the new edit mode manager (father state) - but only when mapRef is available
  const [editModeManager, setEditModeManager] = useState(null);
  
  // Initialize edit mode manager only when mapRef becomes available
  useEffect(() => {
    ...
    if (mapRef.current && !editModeManager) {
      console.log('🏠 HOME: Map ref is now available, initializing edit mode manager');
      const manager = useEditModeManager(mapRef);
      setEditModeManager(manager);
    }
  }, [mapRef, editModeManager]);
