...
export const useMapBehavior = (mapRef, currentMode) => {
  ...
  const handleEditModeClick = useCallback((e) => {
    ...
    // Check if we're clicking on an existing feature (crossroad/destination)
    ...
    // If not snapping to a feature, check if we have mouseenter snap
    if (!isSnapping && snapCrossRef.current) {
    ...
    // If we're not snapping to any feature
    if (!isSnapping) {
