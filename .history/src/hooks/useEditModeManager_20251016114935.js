...
export const useEditModeManager = (mapRef) => {
  ...
  const checkAndSetupEventHandlers = useCallback((mode, handlers = {}) => {
    console.log('🎯 EDIT MODE MANAGER: Checking layer availability for event setup:', layersAvailable);
    ...
    if (allLayersAvailable) {
      console.log('🎯 EDIT MODE MANAGER: All layers available, setting up event handlers');
      setupEventHandlers(mode, handlers);
    } else {
      console.log('🎯 EDIT MODE MANAGER: Waiting for layers to become available...', layersAvailable);
    }
