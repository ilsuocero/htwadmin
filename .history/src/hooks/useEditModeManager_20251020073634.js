...
export const useEditModeManager = (getMap) => {
    ...
    const setupNormalModeHandlers = useCallback((map, handlers) => {
        ...
        try {
            ...
            if (map.getSource('sentieri')) {
                ...
                // NOTE: The backup does NOT set up click handlers for destinations/crossroads
                // Only mousedown handlers are set up, which check editMode.current
                // This prevents popups in EDIT mode
                console.log('🎯 EDIT MODE MANAGER: No click handlers for destinations/crossroads (like backup)');
            ...
            if (map.getLayer('incroci')) {
                ...
                // Hover behavior for incroci (crossroads) - EXACTLY like backup
    ...
    const setupEditModeHandlers = useCallback((map, handlers) => {
        ...
        try {
            ...
            // Also remove any stored click handlers from the ref
            if (eventHandlersRef.current.destinazioniClickHandler) {
                map.off('click', 'destinazioni', eventHandlersRef.current.destinazioniClickHandler);
                eventHandlersRef.current.destinazioniClickHandler = null;
                console.log('🎯 EDIT MODE MANAGER: Edit mode - removed stored destinazioni click handler');
            }
            if (eventHandlersRef.current.incrociClickHandler) {
                map.off('click', 'incroci', eventHandlersRef.current.incrociClickHandler);
                eventHandlersRef.current.incrociClickHandler = null;
                console.log('🎯 EDIT MODE MANAGER: Edit mode - removed stored incroci click handler');
            }
