...
export const useMapBehavior = (mapRef, currentMode) => {
  ...
  const initializeSegmentCreation = useCallback(() => {
    ...
    ['destinazioni', 'incroci'].forEach(layerId => {
      if (map.getLayer(layerId)) {
        ...
        // Click handler for layer-specific clicks
        map.on('click', layerId, (e) => {
          console.log('Layer-specific click on', layerId, 'features:', e.features?.length || 0);
          // This will be handled by the global click handler via e.features
        });
      }
    });
  }, [mapRef]);
  ...
  const handleEditModeClick = useCallback((e) => {
    ...
    console.log('Edit mode click features:', e.features?.length || 0);
    ...
    // SIMPLIFIED APPROACH: Always check for features first
    const clickedFeatures = e.features || [];
    console.log('Clicked features:', clickedFeatures);
    
    // If we have features, use them (this means we clicked on a layer)
    if (clickedFeatures.length > 0) {
      ...
      if (layerId === 'destinazioni' || layerId === 'incroci') {
        ...
        console.log('Clicking on feature:', featureId, clickedCoordinates);
        ...
        if (!currentSnap1) {
          ...
          dispatch({ 
            ...
            payload: { clickedCoords, featureId, featureType: layerId } 
          });
        } else {
      ...
      // No features means we clicked on empty map
      // Check if we have mouseenter snap as fallback
      if (snapCrossRef.current) {
        const { clickedCoordinates, featureId, featureType } = snapCrossRef.current;
        console.log('Using mouseenter snap to feature:', featureId, clickedCoordinates);
        clickedCoords = clickedCoordinates;
        isSnapping = true;
        
        // Store the snap coordinates in global state
        if (!currentSnap1) {
          console.log('First snap to feature (from mouseenter)');
          dispatch({ 
            type: 'SET_EDIT_MODE_SNAP1', 
            payload: { clickedCoords, featureId, featureType } 
          });
        } else {
          console.log('Second snap to feature (from mouseenter)');
          dispatch({ 
            type: 'SET_EDIT_MODE_SNAP2', 
            payload: { clickedCoords, featureId, featureType } 
          });
        ...
        // No features and no mouseenter snap - this is an empty map click
    ...
    const updatedSnap1 = currentSnap1 || state.editMode.snap1;
    const updatedSnap2 = currentSnap2 || state.editMode.snap2;
    
    if (updatedSnap1 && updatedSnap2) {
