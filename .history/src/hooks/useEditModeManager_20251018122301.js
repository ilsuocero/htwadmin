...
export const useEditModeManager = (getMap) => {
  ...
  useEffect(() => {
    ...
    // If layers are already available, don't start monitoring again
    const currentLayersAvailable = {
      sentieri: !!map.getLayer('sentieri'),
      destinazioni: !!map.getLayer('destinazioni'),
      incroci: !!map.getLayer('incroci')
    };
    
    const allLayersCurrentlyAvailable = currentLayersAvailable.sentieri && currentLayersAvailable.destinazioni && currentLayersAvailable.incroci;
    
    if (allLayersCurrentlyAvailable) {
      console.log('🎯🎯🎯🎯🎯🎯 EDIT MODE MANAGER: All layers already available, skipping monitoring');
      // Update state once and exit
      setLayersAvailable(currentLayersAvailable);
      return;
    }
  ...
  useEffect(() => {
    ...
    // Only set pending mode setup if we're actually changing modes
    // This prevents infinite loops when we're already in the same mode
    if (handlersSetupRef.current !== currentMode) {
      console.log('🎯🎯🎯🎯🎯🎯 EDIT MODE MANAGER: Setting pending mode setup to TRUE for mode:', currentMode);
      setPendingModeSetup(true);
    } else {
      console.log('🎯🎯🎯🎯🎯🎯 EDIT MODE MANAGER: Already set up handlers for mode:', currentMode, 'skipping pending setup');
    }
  ...
  useEffect(() => {
    ...
    if (pendingModeSetup && allLayersAvailable) {
      // Check if we've already set up handlers for this mode to prevent infinite loops
      if (handlersSetupRef.current === currentMode) {
        console.log('🎯🎯🎯🎯🎯🎯 EDIT MODE MANAGER: Handlers already set up for mode:', currentMode, 'skipping setup');
  ...
  }, [getMap, pendingModeSetup, layersAvailable, setupEventHandlers, currentMode, handleMapClick, handleContextMenu, handleMapMouseMove, handleMapMouseUp, handleMapMouseDown]);
