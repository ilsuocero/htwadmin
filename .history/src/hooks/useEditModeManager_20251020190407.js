...
export const useEditModeManager = (getMap) => {
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            ...
            // Only set up handlers if we're still in the same mode (prevent race conditions)
            if (handlersSetupRef.current === currentMode) {
                console.log('🎯 EDIT MODE MANAGER: Mode unchanged, setting up delayed handlers');
                setupEventHandlers(currentMode, {
                    click: handleMapClick,
                    contextmenu: handleContextMenu,
                    mousemove: handleMapMouseMove,
                ...
                console.log('🎯 EDIT MODE MANAGER: Mode changed during delay, skipping delayed setup');
            }
