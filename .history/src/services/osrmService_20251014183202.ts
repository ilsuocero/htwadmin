...
export class OSRMService {
  ...
  static async getRoute(
    ...
    profile: string = 'foot'  // This parameter becomes irrelevant for routed-foot
  ): Promise<number[][]> {
    try {
      const [startLng, startLat] = startLngLat;
      const [endLng, endLat] = endLngLat;

      // For routed-foot, always use 'driving' in the URL path
      const url = `${this.BASE_URL}/routed-foot/route/v1/driving/${startLng},${startLat};${endLng},${endLat}`;
      const params = {
        overview: 'full',
        geometries: 'geojson'
