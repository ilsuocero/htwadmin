...
const CrossRoadForm: React.FC<CrossRoadFormProps> = ({
  ...
  const loadPOIDescriptions = async (id: string) => {
    try {
      ...
      if (socket) {
        console.log('HtWCrossRoadForm-->[loadPOIDescriptions]: Using existing socket instance with ID:', socket.id);
        
        socket.emit('getPOI2LangDescr', id, 'en', 'it', (descriptions: any) => {
        ...
        console.error('HtWCrossRoadForm-->[loadPOIDescriptions]: Failed to get socket instance');
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const socket = await getSocketInstance(connectionState);
      if (socket) {
        console.log('HtWCrossRoadForm-->[handleSubmit]: Using existing socket instance with ID:', socket.id);
        
        socket.emit('saveCrossRoad', newFeature, (err: any) => {
          console.log('HtWCrossRoadForm-->[handleSubmit]: saveCrossRoad callback received, error:', err);
          if (!err) {
            console.log('HtWCrossRoadForm-->[handleSubmit]: Crossroad saved successfully');
            onSubmit(newFeature);
            handleClose();
          } else {
            console.error('HtWCrossRoadForm-->[handleSubmit]: Error saving crossroad:', err);
            alert('Failed to save crossroad: ' + (err.message || JSON.stringify(err)));
          }
        });
      } else {
        console.error('HtWCrossRoadForm-->[handleSubmit]: Failed to get socket instance');
