        ...
        if (mapLoaded && destNetwork != null) {
            ...
            if (!map.current.getSource('destinazioni')) {
                ...
                map.current.getSource('destinazioni').setData(destNetwork);
                    ...
                    console.log('🚀 Map-->[useEffect destNetwork]: Skipping hover listeners for destinazioni (auto-segment mode active)');
                }
    ...
    useEffect(() => {
        if (mapLoaded && crossRoads != null) {
            ...
            if (!map.current.getSource('incroci')) {
                ...
                // Only add hover listeners if NOT in auto-segment mode
                if (!autoSegmentModeRef.current) {
                    console.log('🚀 Map-->[useEffect crossRoads]: Adding hover listeners for incroci');
                    map.current.on('mouseenter', 'incroci', function (e) {
                        // Get the ID of the feature under the mouse cursor
                        var featureId = e.features[0].properties.id;
                        if (editMode.current) {
                    ...
                    console.log('🚀 Map-->[useEffect crossRoads]: Skipping hover listeners for incroci (auto-segment mode active)');
                }
