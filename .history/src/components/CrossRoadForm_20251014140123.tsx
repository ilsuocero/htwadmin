...
const CrossRoadForm: React.FC<CrossRoadFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    if (!coordinates) {
      console.error('🚀 HtWCrossRoadForm-->[handleSubmit]: Coordinates are undefined');
      alert('Coordinates are not available. Please try again.');
      return;
    }
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          <Typography variant="caption" color="text.secondary">
            Coordinates: {coordinates?.lng?.toFixed(6) || 'N/A'}, {coordinates?.lat?.toFixed(6) || 'N/A'}
          </Typography>
        </Box>
      </DialogContent>
      <DialogActions>
