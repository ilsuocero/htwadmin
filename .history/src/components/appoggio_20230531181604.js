function newSegment() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('keydown', handleCancelKeyPress);

        return () => {
            window.removeEventListener('keydown', handleCancelKeyPress);
        };
    }, []);
}
...
function deleteLastPoint() {
    const tempPointSource = map.current.getSource('tempPointSource');
    const tempPointData = tempPointSource._data;
    const featuresCount = tempPointData.features.length;
