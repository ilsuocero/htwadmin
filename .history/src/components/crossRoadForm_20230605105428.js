...
export const useCrossRoadForm = (showForm, 
    ...
    useEffect(() => {
        ...
        const saveCrossRoad = async (newFeature) => {
            try {
        ...
        submitButton.addEventListener('click', (e) => {
            ...
            if (selectedFeature.current) {
                ...
                const lng = featureCoordinates[0];
                const lat = featureCoordinates[1];

            } else {
                const lng = showForm.coordinates.lng;
                const lat = showForm.coordinates.lat;
            }
            ...
            const newFeature = {
                ...
                properties: {
                    ...
                    IDF: generateUniqueNumber(),
                    id: idInput.value,
                },
                geometry: {
                    type: 'Point',
                    coordinates: [slng, showForm.coordinates.lat]
                }
            };

            // save into database
            if (connectionState) saveCrossRoad(newFeature);
            //
