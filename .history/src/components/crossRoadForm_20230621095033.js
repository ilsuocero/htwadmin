...
export const useCrossRoadForm = (showForm,
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (socket) {
                    ...
                    socket.on('crossRoadSaved', (err) => {
                        if (!err) {
                            if (['POI', 'POIdest'].includes(sF?.properties?.Tipo)) POI.current = true;
        ...
        const tipoOptions = ['bivio', 'destinazione', 'incrocio', 'POI', 'POIdest', 'indicazione'];
        ...
        // Create dropdown options
        const geoFenceOptions = [30, 50, 100, 200, 500];
        ...
        // Set the selected value
        const geoFenceValue = selectedFeature?.current?.properties?.GeoFence || 30;
        ...
        nameInput.addEventListener('change', (e) => {
            if (['POI', 'POIdest'].includes(typeInput.value)) {
                shortDescriptionLabel2.textContent = 'Sei a ' + nameInput.value + '...' +
                    ' (inizia la descrizione breve con "dove", dove si mangia, dove puoi ammirare...)';
                shortDescriptionLabel.textContent = 'You are at ' + nameInput.value + '...' +
                    ' (start the short description with "where", where you can eat, where you can see...)';
            }
        })
        typeInput.addEventListener('change', (e) => {

            if (['POI', 'POIdest'].includes(e.target.value)) {
                const getDescriptions = async (id) => {
                    try {
                        const socket = await emitEvent(connectionState, 'getPOI2LangDescr', id,
                            PRIMARY_LANGUAGE, SECONDARY_LANGUAGE);
                        if (socket) {
                            //console.log('RV-->[RenderPathNetwork]: socket', socket.id);
                            socket.on('POI2LangDescr', (descriptions) => {
                                shortDescriptionInput.value = descriptions.primary.short;
                                longDescriptionInput.value = descriptions.primary.long;
                                shortDescriptionInput2.value = descriptions.secondary.short;
                                longDescriptionInput2.value = descriptions.secondary.long;
                            });
                        }
                    } catch (error) {
                        alert('Unable to read descriptions:', error)
                        console.error(error);
                    }
                }
                if (connectionState) {
                    getDescriptions(idInput.value)
                }
