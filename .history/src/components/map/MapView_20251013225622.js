...
export const MapView = forwardRef(({
  ...
  // Clear only the layer-specific event listeners we manage
  const clearLayerSpecificListeners = useCallback(() => {
    console.log('🎯 clearLayerSpecificListeners');
    if (!map.current) return;
    
    try {
      // Clear only layer-specific listeners (not global ones that maintain map state)
      ...
      console.warn('MapView: Error clearing layer listeners:', error);
    }
  }, []);
...
// 
  ...
  useEffect(() => {
    ...
    // Add fallback context menu handler
    map.current.on('contextmenu', (e) => {
      console.log('🎯 Global context menu - Mode:', currentMode, 'Point:', e.lngLat);
      e.preventDefault();
      // Test if the context menu callback is working
      if (onContextMenu) {
        console.log('🎯 Calling onContextMenu callback');
        onContextMenu(e);
      }
    });
