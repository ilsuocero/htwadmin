  ...
  const setupNormalModeHandlers = useCallback(() => {
    ...
    try {
      ...
      map.current.on('contextmenu', (e) => {
        ...
        const offsetX = 10;
        const offsetY = -10;
        // e.lngLat contains the map coordinates you want to project to pixels
        const pos = map.current.project(e.lngLat)
          .add(new maplibregl.Point(offsetX, offsetY));
        // pos.x and pos.y now have the offset applied

        setContextMenuState({
          visible: true,
          position: { x: pos.x, y: pos.y }, // Pass these as the position prop
          lngLat: e.lngLat
        });
      });
