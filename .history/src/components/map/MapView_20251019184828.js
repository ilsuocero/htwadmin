...
import { useStateTracker } from '../../hooks/useStateTracker'; // Temporarily disabled
...
const MapView = forwardRef(({
  ...
  onContextMenu,
  currentMode,
  editModeManager // Receive edit mode manager for real-time mode access
}, ref) => {
  const { dispatch } = useAppState();
  
  // Use real-time mode from edit mode manager if available, otherwise fall back to props
  const effectiveCurrentMode = editModeManager?.publicCurrentMode || currentMode;
  if (editModeManager?.publicCurrentMode) {
    console.log(
      `🎯🎯🎯🎯effectiveCurrentMode from editModeManager.publicCurrentMode:`,
      editModeManager.publicCurrentMode
    );
  } else {
    console.log(
      `🎯🎯🎯🎯effectiveCurrentMode from currentMode:`,
      currentMode
    );
  }
  ...
  // Debug props changes
  useEffect(() => {
    console.log('🎯MapView: Props received', {
      pathNetworkFeatures: pathNetwork?.features?.length || 0,
      destNetworkFeatures: destNetwork?.features?.length || 0,
      crossRoadsFeatures: crossRoads?.features?.length || 0,
      currentMode
    });
  }, [pathNetwork, destNetwork, crossRoads, currentMode]);
  
  // Track state changes - temporarily disabled
  // useStateTracker('MapView', {
  //   mapLoaded,
  //   layersLoaded,
  //   currentMode,
  //   hasMap: !!map.current
    ...
    const initializeMap = () => {
      ...
      try {
        ...
        map.current.on('load', () => {
          ...
          console.log('🎯MapView: Map is now ready for layers');
          setMapLoaded(true);
  ...
  const setupNormalModeHandlers = useCallback(() => {
    ...
    try {
      ...
      if (map.current.getSource('sentieri')) {
        console.log('🎯MapView: Setting hover event for sentieri:', effectiveCurrentMode);
        ...
        console.log('🎯MapView: FAILED hover event for sentieri:', effectiveCurrentMode);
            ENV.PATH_OVER || '#ff0000', // Hover color
            ENV.PATH || '#ff000080' // Default color
          ]);
          map.current.getCanvas().style.cursor = 'pointer';
        ...
        console.log('🎯MapView: FAILED mouse event for sentieri:', effectiveCurrentMode);
      }
// 
      if (map.current.getLayer('destinazioni')) {
        console.log('🎯MapView: Setting mouse event for destinazioni:', effectiveCurrentMode);
        if (onMapMouseDown) {
        ...
        console.log('🎯MapView: FAILED mouse event for destinazioni:', effectiveCurrentMode);
      }
      // 
      if (map.current.getLayer('incroci')) {
        console.log('🎯MapView: Setting mouse event for incroci:', effectiveCurrentMode);
        if (onMapMouseDown) {
        ...
        console.log('🎯MapView: FAILED mouse event for incroci:', effectiveCurrentMode);
      }
      // 
    } catch (error) {
      console.error('MapView: Error setting up normal handlers:', error);
    }
  }, [effectiveCurrentMode, onContextMenu, onMapMouseDown, onMapMouseMove, onMapMouseUp]);
  
  // Edit mode handlers - ONLY for crossroads/destinations
    ...
    try {
      ...
      if (onMapClick) {
        ...
        if (map.current.getLayer('destinazioni')) {
          ...
          console.log('🎯MapView: Edit mode - destinazioni click handler added');
        }
        if (map.current.getLayer('incroci')) {
          map.current.on('click', 'incroci', onMapClick);
          console.log('🎯MapView: Edit mode - incroci click handler added');
        }
        // Add global click for intermediate points (empty map areas)
        map.current.on('click', onMapClick);
        console.log('🎯MapView: Edit mode - global click handler added');
      }
      ...
      if (map.current.getLayer('sentieri')) {
        ...
        map.current.setPaintProperty('sentieri', 'line-color', ENV.PATH || '#ff000080');
        console.log('🎯MapView: Edit mode - segment interactions removed and visual effects disabled');
      }
    } catch (error) {
  ...
  // Event handler management - father state manages ALL events automatically through coordinated effect
  useEffect(() => {
    if (!map.current || !mapLoaded || !editModeManager) return;
    
    console.log('🚀 MapView: Father state automatically managing event handlers for mode:', effectiveCurrentMode);
    ...
    // Father state now automatically manages event handlers through the coordinated effect
    // No need to manually call setup functions - the father state handles it automatically
    console.log('🚀 MapView: Event handlers are now managed automatically by father state');
    
    // Add fallback global click handler for debugging (only in NORMAL mode)
    if (effectiveCurrentMode === 'NORMAL') {
      map.current.on('click', (e) => {
        console.log('🔍 Global map click - Mode:', effectiveCurrentMode, 'Features:', e.features?.length || 0);
        console.log('🔍 Global map click - Point:', e.lngLat);
        console.log('🔍 Global map click - Available layers:', {
          sentieri: layersLoaded.sentieri,
          destinazioni: layersLoaded.destinazioni,
          incroci: layersLoaded.incroci
        });
      });
    } else {
      // Remove global click handler in non-NORMAL modes to avoid interference
      console.log('🚀 MapView: Global click handler disabled for mode:', effectiveCurrentMode);
    }
    
    // Debug: Check if hover handlers are working
    console.log('🔍 MapView: Checking hover handler status', {
      hasSentieriLayer: !!map.current.getLayer('sentieri'),
      hasDestinazioniLayer: !!map.current.getLayer('destinazioni'),
      hasIncrociLayer: !!map.current.getLayer('incroci'),
      currentMode: effectiveCurrentMode
    });
    
  }, [mapLoaded, effectiveCurrentMode, layersLoaded.sentieri, layersLoaded.destinazioni, layersLoaded.incroci, editModeManager, onMapClick, onContextMenu, onMapMouseDown, onMapMouseMove, onMapMouseUp]);
  
  // Data change detection - track when data changes from null to actual data
  ...
  // Monitor when data changes from null to actual data
  useEffect(() => {
    const newDataState = {
      pathNetwork: !!pathNetwork && pathNetwork.features?.length > 0,
      destNetwork: !!destNetwork && destNetwork.features?.length > 0,
      crossRoads: !!crossRoads && crossRoads.features?.length > 0
    };

    console.log('🎯MapView: Data state changed', newDataState);
    console.log('🎯MapView: Data details -', {
      pathNetworkFeatures: pathNetwork?.features?.length || 0,
      destNetworkFeatures: destNetwork?.features?.length || 0,
      crossRoadsFeatures: crossRoads?.features?.length || 0
    });
    
    if (JSON.stringify(newDataState) !== JSON.stringify(dataLoaded)) {
      console.log('🎯MapView: Updating dataLoaded state from', dataLoaded, 'to', newDataState);
      setDataLoaded(newDataState);
    } else {
      console.log('🎯MapView: dataLoaded state unchanged', dataLoaded);
    }
  }, [pathNetwork, destNetwork, crossRoads]); // Remove dataLoaded from dependencies to prevent infinite loop

  // Layer management - with center preservation
  useEffect(() => {
    console.log('🎯MapView: useEffect for pathNetwork triggered', {
      mapLoaded,
      hasPathNetwork: !!pathNetwork,
      hasMap: !!map.current,
      pathNetworkFeatures: pathNetwork?.features?.length || 0,
      dataLoaded: dataLoaded.pathNetwork
    });
    
    if (!mapLoaded || !pathNetwork || !map.current || !dataLoaded.pathNetwork) return;
    
    try {
      const currentCenter = map.current.getCenter();
      const currentZoom = map.current.getZoom();
      
      console.log('🎯MapView: Adding sentieri layer with', pathNetwork.features.length, 'features');
      ...
      if (!map.current.getSource('sentieri')) {
        ...
        console.log('🎯MapView: Sentieri source data updated');
      }
  ...
  }, [mapLoaded, pathNetwork, dataLoaded.pathNetwork]); // Add dataLoaded.pathNetwork dependency
  
  useEffect(() => {
    console.log('🎯MapView: useEffect for destNetwork triggered', {
      mapLoaded,
      hasDestNetwork: !!destNetwork,
      hasMap: !!map.current,
      destNetworkFeatures: destNetwork?.features?.length || 0,
      dataLoaded: dataLoaded.destNetwork
    });
    
    if (!mapLoaded || !destNetwork || !map.current || !dataLoaded.destNetwork) return;
    
    try {
      const currentCenter = map.current.getCenter();
      const currentZoom = map.current.getZoom();
      
      console.log('🎯MapView: Adding destinazioni layer with', destNetwork.features.length, 'features');
      ...
      if (!map.current.getSource('destinazioni')) {
        ...
        console.log('🎯MapView: Destinazioni source data updated');
      }
  ...
  }, [mapLoaded, destNetwork, dataLoaded.destNetwork]); // Add dataLoaded.destNetwork dependency
  
  useEffect(() => {
    console.log('🎯MapView: useEffect for crossRoads triggered', {
      mapLoaded,
      hasCrossRoads: !!crossRoads,
      hasMap: !!map.current,
      crossRoadsFeatures: crossRoads?.features?.length || 0,
      dataLoaded: dataLoaded.crossRoads
    });
    
    if (!mapLoaded || !crossRoads || !map.current || !dataLoaded.crossRoads) return;
    
    try {
      const currentCenter = map.current.getCenter();
      const currentZoom = map.current.getZoom();
      
      console.log('🎯MapView: Adding incroci layer with', crossRoads.features.length, 'features');
      ...
      if (!map.current.getSource('incroci')) {
        ...
        console.log('🎯MapView: Incroci source data updated');
      }
  ...
  }, [mapLoaded, crossRoads, dataLoaded.crossRoads]); // Add dataLoaded.crossRoads dependency
  
  // Combined layer management - triggers when both map is loaded AND data is available
  useEffect(() => {
    console.log('🎯MapView: Combined layer management triggered', {
      mapLoaded,
      dataLoaded,
      allDataReady: dataLoaded.pathNetwork && dataLoaded.destNetwork && dataLoaded.crossRoads,
      hasMap: !!map.current
    });
    ...
    const allDataReady = dataLoaded.pathNetwork && dataLoaded.destNetwork && dataLoaded.crossRoads;
    
    if (allDataReady) {
      console.log('🎯MapView: All data ready, adding layers to loaded map');
      
      // Force re-trigger of individual layer management hooks
      // This ensures layers are added now that both map and data are ready
      setTimeout(() => {
        console.log('🎯MapView: Forcing layer management hooks to re-run');
        // The individual useEffect hooks will now run with mapLoaded=true and data available
      }, 100);
    }
  }, [mapLoaded, dataLoaded.pathNetwork, dataLoaded.destNetwork, dataLoaded.crossRoads]);
