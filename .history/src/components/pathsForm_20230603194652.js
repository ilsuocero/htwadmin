...
export const usePathsForm = (pathForm, setPathForm, setPathNetwork, map, mapContainer) => {
    useEffect(() => {
        // Your code here
        if (!pathsForm.show) {
            return; // Don't create the form if pathsForm is false
        }
        ...
        submitButton.addEventListener('click', (e) => {
            ...
            const newFeature = {
                ...
                geometry: {
                    type: 'Point',
                    coordinates: [pathsForm.coordinates.lng, pathsForm.coordinates.lat]
                }
            };
        ...
        const pos = map.current.project(pathsForm.coordinates).add(new maplibregl.Point(offsetX, offsetY));
        ...
        // Cleanup function
        return () => {
            // Remove the form when the component unmounts or `pathsForm` changes to `false`
    ...
    }, [pathsForm.show, pathsForm.coordinates]);
};