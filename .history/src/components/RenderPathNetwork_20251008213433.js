...
function RenderPathNetwork({setPathNetwork, connectionState }) {
    ...
    React.useEffect(() => {
        console.log('RV-->[RenderPathNetwork]: connectionState changed', JSON.stringify(connectionState));
        
        const fetchSentieri = async () => {
            try {
                console.log('RV-->[RenderPathNetwork]: Attempting to fetch sentieri...');
                const socket = await emitEvent(connectionState, 'listaSentieri');
                if (socket) {
                    console.log('RV-->[RenderPathNetwork]: Socket connected with ID:', socket.id);
