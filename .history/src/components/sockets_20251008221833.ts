...
export const emitEvent = async (
  ...
  ...args: any[]
    ...
    socketInstance.on('connect', () => {
        ...
        if (connectionState.tokenIO && socketInstance) {
...
export const getSocketInstance = async (
  connectionState: ConnectionState
    ...
    if (!socketInstance) {
        ...
        socketInstance.on('connect', () => {
            console.log('HtWsockets-->[getSocketInstance connect]: Socket.io connected successfully, ID:', socketInstance?.id);
            // Authenticate after connection is established
            if (connectionState.tokenIO && socketInstance) {
                console.log('HtWsockets-->[getSocketInstance connect]: Authenticating with token');
                socketInstance.emit('authenticate', { token: connectionState.tokenIO });
            }
        });
    }
