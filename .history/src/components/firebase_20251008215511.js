...
const getIdToken = async (user) => {
    try {
        if (user) {
            ...
            console.log('HtWfirebase-->[getIdToken]: Firebase ID token retrieved:', token ? 'Yes' : 'No');
            return token;
        }
        return null;
    } catch (error) {
        console.error('HtWfirebase-->[getIdToken]: Error getting Firebase ID token:', error);
        return null;
    }
};

const readToken = (setToken) => {
