...
const Map = forwardRef(({
    ...
    const startAutoSegmentMode = () => {
        ...
        console.log('🚀 Map-->[startAutoSegmentMode]: Current autoSegmentMode state:', autoSegmentMode);
        console.log('🚀 Map-->[startAutoSegmentMode]: Current autoSegmentModeRef:', autoSegmentModeRef.current);
        ...
        // Disable ALL hover effects for crossroads and destinations during auto-segment mode
        console.log('🚀 Map-->[startAutoSegmentMode]: Disabling ALL hover effects');
        ...
        // Also disable the snapCrossRef functionality during auto-segment mode
        ...
        console.log('🚀 Map-->[startAutoSegmentMode]: Updated autoSegmentMode state:', true);
        console.log('🚀 Map-->[startAutoSegmentMode]: Updated autoSegmentModeRef:', autoSegmentModeRef.current);
    };

    const stopAutoSegmentMode = () => {
        try {
            console.log('🚀 Map-->[stopAutoSegmentMode]: Stopping auto-segment mode');
            console.log('🚀 Map-->[stopAutoSegmentMode]: Current autoSegmentMode state:', autoSegmentMode);
            console.log('🚀 Map-->[stopAutoSegmentMode]: Current autoSegmentModeRef:', autoSegmentModeRef.current);
            ...
            console.log('🚀 Map-->[stopAutoSegmentMode]: State updated, updating cursor');
            if (map.current) {
            ...
            console.log('🚀 Map-->[stopAutoSegmentMode]: Normal mode interactions will be restored by centralized system');
            // DO NOT re-enable interactions here - they will be handled by the centralized system
            // The centralized useEffect runs on every render and will restore normal mode behavior
            
            console.log('🚀 Map-->[stopAutoSegmentMode]: Removing auto-segment click listeners');
            // Remove click listener for destinazioni
            if (map.current) {
                map.current.off('click', 'destinazioni', handleCrossroadClick);
                map.current.off('click', 'incroci', handleCrossroadClick);
            }
            
            console.log('🚀 Map-->[stopAutoSegmentMode]: Resetting crossroad colors to normal');
            // Reset crossroad colors to normal
            if (map.current) {
            ...
            console.log('🚀 Map-->[stopAutoSegmentMode]: Removing any temporary layers');
            ...
            console.log('🚀 Map-->[stopAutoSegmentMode]: Updated autoSegmentMode state:', false);
            console.log('🚀 Map-->[stopAutoSegmentMode]: Updated autoSegmentModeRef:', autoSegmentModeRef.current);
            
        } catch (error) {
            console.error('🚀 Map-->[stopAutoSegmentMode]: ERROR during cleanup:', error);
            console.error('🚀 Map-->[stopAutoSegmentMode]: Error stack:', error.stack);
            
            // Force cleanup even if there's an error
            autoSegmentModeRef.current = false;
            setAutoSegmentMode(false);
            setSelectedCrossroads([]);
            setLoadingOSRM(false);
    ...
    // Track autoSegmentMode state changes
    useEffect(() => {
        console.log('🚀 Map-->[useEffect autoSegmentMode]: State changed to:', autoSegmentMode);
        console.log('🚀 Map-->[useEffect autoSegmentMode]: Ref value:', autoSegmentModeRef.current);
    }, [autoSegmentMode]);

    // Centralized map behavior management
    const clearAllMapListeners = () => {
        if (!map.current) return;
        
        console.log('🚀 Map-->[clearAllMapListeners]: Clearing all map listeners');
    ...
    // Named function for segment click to prevent multiple handlers
    ...
    const setupNormalMode = () => {
        if (!map.current) return;
        
        console.log('🚀 Map-->[setupNormalMode]: Setting up normal mode behavior');
        
        // Context menu
        map.current.on('contextmenu', handleContextMenu);
        
        // Segment interactions - use named function to prevent multiple handlers
        map.current.on('click', 'sentieri', handleSegmentClick);
        ...
        // Point interactions (destinazioni and incroci)
    ...
    const setupAutoSegmentMode = () => {
        if (!map.current) return;
        
        console.log('🚀 Map-->[setupAutoSegmentMode]: Setting up auto-segment mode behavior');
        
        // Set crosshair cursor
        map.current.getCanvas().style.cursor = 'crosshair';
        
        // Add click listeners for crossroads selection
        map.current.on('click', 'destinazioni', handleCrossroadClick);
        map.current.on('click', 'incroci', handleCrossroadClick);
        
        // EXPLICITLY DISABLE ALL HOVER EFFECTS during auto-segment mode
        // No segment hover effects
        map.current.off('mouseenter', 'sentieri');
        map.current.off('mouseleave', 'sentieri');
        
        // No point hover effects
        map.current.off('mouseenter', 'destinazioni');
        map.current.off('mouseleave', 'destinazioni');
        map.current.off('mouseenter', 'incroci');
        map.current.off('mouseleave', 'incroci');
        
        // Disable all other interactions during auto-segment mode
    ...
    const setupEditMode = () => {
        ...
        console.log('🚀 Map-->[setupEditMode]: Setting up edit mode behavior');
        ...
        // Keep hover effects for snap functionality
        map.current.on('mouseenter', 'destinazioni', function (e) {
            var featureId = e.features[0].properties.id;
            const clickedCoordinates = e.features[0].geometry.coordinates;
            snapCrossRef.current = { clickedCoordinates, featureId };
        ...
        // Keep point dragging functionality
        map.current.on('mousedown', 'destinazioni', handlePointsMouseDown);
        map.current.on('mousedown', 'incroci', handlePointsMouseDown);
    };
    ...
    const applyVisualStyles = () => {
        ...
        console.log('🚀 Map-->[applyVisualStyles]: Applying visual styles');
        
        // Apply crossroad selection colors if in auto-segment mode
    ...
    // Centralized map behavior management - runs on EVERY render
    useEffect(() => {
        if (!map.current || !mapLoaded) return;
        
        console.log('🚀 Map-->[useMapBehavior]: Managing map behavior on EVERY render');
        console.log('🚀 Map-->[useMapBehavior]: autoSegmentMode:', autoSegmentModeRef.current);
        console.log('🚀 Map-->[useMapBehavior]: editMode:', editMode.current);
    ...
    const generateAutoSegment = async (crossroads) => {
        ...
        try {
            ...
            console.log('🚀 Map-->[generateAutoSegment]: Creating temporary route layer');
            ...
            console.log('🚀 Map-->[generateAutoSegment]: Opening PathForm');
            ...
            console.log('🚀 Map-->[generateAutoSegment]: PathForm opened, stopping auto-segment mode');
    ...
    // Edit Mode - Context menu management is now handled by centralized behavior system
    // This useEffect is redundant and conflicts with the centralized system
    ...
    useEffect(() => {
        if (mapLoaded && pathNetwork != null) {
            ...
            if (!map.current.getSource('sentieri')) {
                ...
                // Segment interactions will be managed by the centralized behavior system
                // Do NOT add event listeners here - they will be added in setupNormalMode()
            } else {
    ...
    useEffect(() => {
        if (mapLoaded && destNetwork != null) {
            ...
            if (!map.current.getSource('destinazioni')) {
                ...
                // Point interactions will be managed by the centralized behavior system
                // Do NOT add event listeners here - they will be added in setupNormalMode() or setupEditMode()
            } else {
                // Update the source data if it already exists
    ...
    const handlePointsMouseDown = useCallback((e) => {
        if (editMode.current) return;
        if (autoSegmentModeRef.current) return; // Skip if in auto-segment mode (use ref for immediate access)
        if (!e) return
        if (!editMode.current) {
            console.log('RV-->[map]: handlePointsMouseDown, editMode.current:', editMode.current);
            e.preventDefault();
            selectedFeature.current = e.features[0];
            // Start dragging - these events are managed by the centralized system
            // They will be cleared and re-applied on every render
        }

    }, []);

    // Add dragging events to the centralized system
    const setupDraggingMode = () => {
        if (!map.current || !selectedFeature.current) return;
        
        console.log('🚀 Map-->[setupDraggingMode]: Setting up dragging mode');
    ...
    return (
        <div className="map-wrap">
            ...
            <PathForm
                open={pathForm.show}
                onClose={() => {
                    console.log('🚀 Map-->[PathForm onClose]: PathForm closed, stopping auto-segment mode');
