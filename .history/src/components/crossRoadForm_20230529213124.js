...
export const useCrossRoadForm = (showForm, setShowForm, setCrossRoads, map, mapContainer) => {
    useEffect(() => {
        // Your code here
        if (!showForm.show) {
        ...
        const tipoOptions = ['destinazione', 'incrocio', 'bivio', 'POI'];
...
By creating a select element(geoFenceInput) and populating it with option elements for each value in the geoFenceOptions array, you'll have a dropdown input field with the desired standard values.

Please replace the existing code for the geoFenceInput element with the updated code snippet provided above.
