...
export const getSocketInstance = async (
  connectionState: ConnectionState
    ...
    // Check if existing socket instance is valid
    if (socketInstance) {
    ...
    socketInstance.on('reconnect', (attemptNumber) => {
        console.log('HtWsockets-->[getSocketInstance reconnect]: Socket reconnected, attempt:', attemptNumber);
    });

    socketInstance.on('reconnect_error', (error) => {
        console.error('HtWsockets-->[getSocketInstance reconnect_error]: Socket reconnection error:', error);
    });
